      ...
      if (isConnected()) {
        if (isSensorSupported(componentName.trim())) {
          try {            
            ...
            // byte[] readData = deviceProtocol.Read(componentName, aux);
...
//  
  ...
  protected String formatReading(String sensorName, byte[] readData) {
    //  int result = (readData[0] & 0xFF); 
     //Test --> Values are a four byte array float
    //  float result = ByteBuffer.wrap(readData).getFloat();            
     logger.debug(" Value {}", readData[0] << 24 | (readData[1] & 0xFF) << 16 | (readData[2] & 0xFF) << 8 | (readData[3] & 0xFF));     
      //Test
    //  return String.valueOf(result);
    return Float.toString(readData[0] << 24 | (readData[1] & 0xFF) << 16 | (readData[2] & 0xFF) << 8 | (readData[3] & 0xFF));    
